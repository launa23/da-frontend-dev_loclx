...
function getIncomeTransactions(tk, dateFrom, dateTo, str) {
    $.ajax({
        url: "http://localhost:8080/transaction/statistic/outcome?start_date=" + dateFrom + "&end_date=" + dateTo + "&type=income" + "&cate=" + str,
        type: "GET",
        headers: {
            "Authorization": tk
        },
...
function fillCategories(tk){
    $.ajax({
        ...
        success: function(response) {
            ...
            console.log(dataCategories[0].child.length);
            for(var i=0; i<dataCategories.length; i++){
                if(dataCategories[i].child.length == 0){
                    var htmlSnippet = `
                        <li style = "margin-bottom: 4px;">
                            <div class="img-icon">
                                <img src="${dataCategories[i].icon}" alt="">
                            </div>
                            <option class = "DA-opIcon" value=${dataCategories[i].id}>${dataCategories[i].name}</option>
                        </li>
                        `;
                    $('#DA-listCategories').append(htmlSnippet);
                } else {
                    var childCategories = dataCategories[i].child.map((cate) => {
                        if(cate.active == true){
                            return {
                                name: cate.name,
                                id: cate.id,
                    ...
                    console.log(childCategories.length)
                    var htmlSnippet1 = `
                        <li style = "margin-bottom: 4px;" id = "DA-${dataCategories[i].id}">
                            <div>
                                <div class="img-icon">
                                    <img src="${dataCategories[i].icon}" alt="">
                                </div>
                            ...
                            <ul class="DA-sub-menu mt-1">
                            
                            </ul>
                        </li>
                        `;
                    $('#DA-listCategories').append(htmlSnippet1);
                    console.log(childCategories);
                    for(var j=0; j<childCategories.length; j++){
                        if(childCategories[j] != undefined){
                            var htmlSnippet = `
                            <li style = "margin-bottom: 4px;">
                                <div class="img-icon">
                                    <img src="${childCategories[j].icon}" alt="">
                                </div>
                                <option class = "DA-opIcon" value=${childCategories[j].id}>${childCategories[j].name}</option>
                            </li>
                            `;
                        $(`#DA-${dataCategories[i].id} ul`).append(htmlSnippet);
                        }
...
function renderOutcomeTransactionToday(dataIncomeTransToday) {
